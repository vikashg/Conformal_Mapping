...
for j in "${roi[@]}"
do
    trk_file=${base_dir}/${subj}/NewResults/Binary_Trk_2_Cell/Resampled_augmented_convol/$j$extension
    map_file=${base_dir}/${subj}/NewResults/Mapped_tracks_binary/Mapped_tracks_$j$extension
    echo $map_file
    if [ -f $trk_file ] && [ ! -f $map_file ]; then 
    echo $trk_file
    qsub -v extension=${extension},subj=${subj},roi=$j,input_file=$input_file ${apps_dir}/Map_tracks_atomic_training_data.sh
    fi
done
